...
<script>
// Port Vue 3 dari apps/components/vue/gov2formfield.vue (#6118 3b).
// Perubahan sadar:
// - v-for + v-if satu elemen → <template v-for> (presedensi Vue 3)
// - this.$scrollTo (vue-scrollto, Vue 2 only) → scrollIntoView native
// - gov2component tetap runtime-load per-app (jalur vue3-sfc-loader) dgn
//   resolusi path yang sama seperti sebelumnya (gov2login/user khusus)
// - childrenData dideklarasikan di data (dulu implicit)
import eventBus from '../eventBus.js'
import { loadVue } from '../loader.js'

var apppath = window.location.pathname;
var n = apppath.search("gov2login/user");
var gov2componentUrl;
if (n == 1) {
    gov2componentUrl = window.location.href + '/../../vue/gov2component.vue';
} else {
    gov2componentUrl = window.location.href + '/../vue/gov2component.vue';
}
...
export default {
    name: 'formfield',
      components: {
        'gov2component': loadVue(gov2componentUrl),
      },
    props: {
        fieldUrl: String,
        action: String,
