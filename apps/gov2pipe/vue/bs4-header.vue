  ...
  <div class="bs4-header">
    <header class="navbar navbar-expand-lg navbar-light container d-block d-lg-flex" id="header-navbar">
      <a class="navbar-brand float-left float-lg-none" href="/" id="logo" style="text-align:center">
        <img alt="" class="normal-logo logo-white" src="../../../images/logo-bkn.png"/>
      ...
      <ul class="nav navbar-nav mr-auto d-none d-lg-block mrg-l-none">
        ...
        <!--		        	<bs4-menu-item-->
        <!--		        		:name="myTogels[2].name"-->
        <!--		        		:status="myTogels[2].status"-->
        <!--		        		:d="myData2"-->
        <!--						:root="root"-->
        <!--		        	></bs4-menu-item>-->
        <!--		        </template>-->

        <template v-if="loadChild && !showPortalList">
          <bs4-menu-settings
        ...
        <template v-if="loadChild">
          <bs4-menu-portal v-if="showPortalList"
      ...
      <ul class="nav navbar-nav ml-auto float-right float-lg-none" id="header-nav" v-if="accId > 0 || String(accId).length > 0">
        <li class="dropdown profile-dropdown" :class="[myTogels[3].status ? 'show' : '']">
          ...
          <ul class="dropdown-menu dropdown-menu-right" :class="[myTogels[3].status ? 'show' : '']">
            ...
            <li>
              <a :href="root+'/'+myFolder+'/privilege/krisna'">
                <i class="fa fa-key">
                </i>
                Privilege
              </a>
...
module.exports = {
  ...
  props: ['getUrl','root','accId','fullname','domain','cmdId', 'role', 'portalName', 'unitName'],
  methods: {
    togel: function(n) {
  ...
  computed: {
    ...
    showPortalList() {
      const forbidden = ['renobiro.bkn.kl2.web.id', 'reno.bkn.go.id',
        'inspektorat.bkn.kl2.web.id', 'inspektorat.bkn.go.id'];
